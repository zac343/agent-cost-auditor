...
```bash
npm exec --yes --package=github:zac343/agent-cost-auditor#v0.5.2 -- agent-cost-audit --auto codex
npm exec --yes --package=github:zac343/agent-cost-auditor#v0.5.2 -- agent-cost-audit --auto claude-code
npm exec --yes --package=github:zac343/agent-cost-auditor#v0.5.2 -- agent-cost-audit --auto openclaw
```

Or audit an explicit export or directory:

```bash
npm exec --yes --package=github:zac343/agent-cost-auditor#v0.5.2 -- agent-cost-audit ./usage.jsonl
```

Audit the newest compatible Codex rollouts in a sessions directory, optionally centered on one session ID or filename:

```bash
npm exec --yes --package=github:zac343/agent-cost-auditor#v0.5.2 -- agent-cost-audit ~/.codex/sessions --session SESSION_ID
```

Piped JSON, JSONL, or CSV is also accepted:

```bash
cat usage.json | npm exec --yes --package=github:zac343/agent-cost-auditor#v0.5.2 -- agent-cost-audit --format json -
```
...
npm exec --yes --package=github:zac343/agent-cost-auditor#v0.5.2 -- agent-cost-audit session.json
```
...
```text
https://mailcheck.agentcartai.com/tools/agent-cost-auditor/auditor.mjs?v=38
```
